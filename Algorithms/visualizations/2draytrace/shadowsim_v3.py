...
class LineSegment:
    ...
    def draw(self):
        pygame.draw.aaline(minimap, color_white, (self.x1, self.y1),
                           (self.x2, self.y2))
...
class Ray:
    ...
    def closest_intersect(self, lss: [LineSegment]):
        return functools.reduce(
            lambda a, b: min(dist(a, (self.x1, self.y1)), dist(b, (self.x1, self.y1))),
            filter(lambda x: x is not None, map(self.intersect, lss))
        )

    def closest_intersect2(self, lss: [LineSegment]):
        closest = (10000, 10000)
...
class RaySource:
    ...
    def draw(self):
        pygame.draw.circle(minimap, color_red, (self.x, self.y), 10)
        pygame.draw.aaline(minimap, color_red, (self.x, self.y),
                           ((math.cos(self.heading) * 20 + self.x),
                            (math.sin(self.heading) * 20 + self.y)))
...
while run:
    ...
    minimap = pygame.Surface(WINDOW_RESOLUTION)
    ...
    pygame.draw.rect(display, color_black,
                     [0, 0, WINDOW_RESOLUTION[0], WINDOW_RESOLUTION[1]])

    for obstacle in obstacles:
        obstacle.draw()

    for ray in rays:
        ray.draw(ray.closest_intersect2(obstacles))

    ray_source.draw()

    display.blit(pygame.transform.scale(minimap, (int(WINDOW_RESOLUTION[0] * minimap_ratio), int(WINDOW_RESOLUTION[1] * minimap_ratio))), (0, 0))
