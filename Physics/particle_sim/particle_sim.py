...
while run:
    ...
    for ball in balls:
        if ball.collides_with_ball(balls):
            ball.bounce(ball.collides_with_ball(balls), elasticity * 0 + 1)

        if ball.y - ball.radius <= 0 or ball.y + ball.radius >= WINDOW_RESOLUTION[1]:
            ball.y_vel *= -elasticity
            ball.update_position(dt)

        if ball.x - ball.radius <= 0 or ball.x + ball.radius >= WINDOW_RESOLUTION[0]:
            ball.x_vel *= -elasticity
            ball.update_position(dt)

        if ball.x <= -ball.radius or ball.x >= WINDOW_RESOLUTION[0] + ball.radius:
            ball.reset(WINDOW_RESOLUTION, -256, 256)
